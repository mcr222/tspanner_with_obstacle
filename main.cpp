...
struct edge {
	...
	//tested
	bool operator ==(const edge& edg) const {
		return ((edg.p1 == p1 && edg.p2 == p2) || (edg.p1 == p2 && edg.p2 == p1));
	}
};
...
struct event {
	...
	bool operator <(const event& evt) const {
		if (angle == evt.angle) {
			return (euclidean_distance(getpoint(p), getpoint(p_view))
					< euclidean_distance(getpoint(evt.p), getpoint(evt.p_view)));
		}
...
double dijkstra_shortest_path(graph& tspanner, int source, int target,
	...
	while (!pq.empty()) {
		...
		if (!visited[prior_point.second]) {
			...
			for (set<int>::iterator it = neighbors.begin();
					it != neighbors.end(); ++it) {
				alt_dist = distances[prior_point.second]
						+ euclidean_distance(getpoint(prior_point.second),
								getpoint(*it));
...
bool edge_intersects_obstacle(int p1, int p2) {
	if (edges_intersect(getpoint(p1), getpoint(p2),
			getpoint(pr.n + pr.nobs - 1), getpoint(pr.n))) {
		return true;
	}
	for (int i = 1; i < pr.nobs; ++i) {
		if (edges_intersect(getpoint(p1), getpoint(p2), getpoint(pr.n + i - 1),
				getpoint(pr.n + i))) {
			return true;
		}
	}
...
bool same_direction_vectors(coord v1, coord v2) {
	return (v1.second * v2.first == v1.first * v2.second
			&& (v1.first * v2.first + v1.second * v2.second > 0));
}
//tested
...
void insert_segment_events(vector<event>& events, int i, int p, int obst1,
		int obst2, coord infinity_point) {
	event evt1;
	event evt2;
	edge segment;
	segment.p1 = obst1;
	...
	evt1.p_view = p;
	evt2.p_view = p;
	if (edges_intersect(getpoint(p), infinity_point, getpoint(obst1),
			getpoint(obst2)) && evt1.angle != 0 && evt2.angle != 0) {
		if (evt1.angle < evt2.angle) {
			...
			evt1.starting = true;
			evt2.starting = false;
		} else if (euclidean_distance(getpoint(evt1.p), getpoint(p))
				< euclidean_distance(getpoint(evt2.p), getpoint(p))) {
			evt1.starting = true;
			evt2.starting = false;
		} else {
			evt1.starting = false;
...
coord find_intersection_lines(coord p1, coord p2, coord v1, coord v2) {
	...
	double n1, d, n2;
	n1 = ((p1.first * p2.second - p1.second * p2.first) * (v1.first - v2.first)
			- (p1.first - p2.first)
					* (v1.first * v2.second - v1.second * v2.first));
	n2 = ((p1.first * p2.second - p1.second * p2.first)
			* (v1.second - v2.second)
			- (p1.second - p2.second)
					* (v1.first * v2.second - v1.second * v2.first));
	d = ((p1.first - p2.first) * (v1.second - v2.second)
			- (p1.second - p2.second) * (v1.first - v2.first));
...
bool point_inside_obstacle(coord p) {
	...
	int inters = 0;
	coord intersection;
	if (edges_intersect(p, infinity_point, getpoint(pr.n + pr.nobs - 1),
			getpoint(pr.n))) {
		if (!point_in_segment(p, getpoint(pr.n + pr.nobs - 1),
				getpoint(pr.n))) {
			++inters;
		} else {
			return true;
	...
	for (int i = 1; i < pr.nobs; ++i) {
		if (edges_intersect(p, infinity_point, getpoint(pr.n + i - 1),
				getpoint(pr.n + i))) {
			if (!point_in_segment(p, getpoint(pr.n + pr.nobs - 1),
					getpoint(pr.n))) {
				++inters;
...
struct status_segment {
	...
	bool operator <(const status_segment seg) const {
		...
		int k = -20;
		if ((seg.p1 == p1 && seg.p2 == p2) || (seg.p1 == p2 && seg.p2 == p1)) {
		...
		if (seg.p1 != p1 && seg.p1 != p2) {
			if (edges_intersect(getpoint(p), getpoint(seg.p1), getpoint(p1),
					getpoint(p2))) {
				...
				inters = find_intersection_lines(getpoint(p), getpoint(seg.p1),
						getpoint(p1), getpoint(p2));
				if (p == k) {
					cout << "inters " << inters.first << " " << inters.second
							<< endl;
					cout << "p " << getpoint(p).first << " "
							<< getpoint(p).second << endl;
					cout << "p2 " << getpoint(p2).first << " "
							<< getpoint(p2).second << endl;
					cout << "seg.p1 " << getpoint(seg.p1).first << " "
							<< getpoint(seg.p1).second << endl;
					cout << "seg.p2 " << getpoint(seg.p2).first << " "
							<< getpoint(seg.p2).second << endl;
					cout << "positive int "
							<< positive_intersection(getpoint(p),
									getpoint(seg.p1), inters) << endl;
					cout << "point in seg"
							<< point_in_segment(inters, getpoint(p1),
									getpoint(p2)) << endl;
				}
		...
		if (seg.p2 != p1 && seg.p2 != p2) {
			if (edges_intersect(getpoint(p), getpoint(seg.p2), getpoint(p1),
					getpoint(p2))) {
				...
				inters = find_intersection_lines(getpoint(p), getpoint(seg.p2),
						getpoint(p1), getpoint(p2));
		...
		if (p1 != seg.p1 && p1 != seg.p2) {
			if (edges_intersect(getpoint(p), getpoint(p1), getpoint(seg.p1),
					getpoint(seg.p2))) {
				if (p == k) {
					cout << "1st false" << endl;
				}
				...
				inters = find_intersection_lines(getpoint(p), getpoint(p1),
						getpoint(seg.p1), getpoint(seg.p2));
				if (positive_intersection(getpoint(p), getpoint(p1), inters)
						&& point_in_segment(inters, getpoint(seg.p1),
								getpoint(seg.p2))) {
					if (p == k) {
		...
		if (p2 != seg.p1 && p2 != seg.p2) {
			if (edges_intersect(getpoint(p), getpoint(p2), getpoint(seg.p1),
					getpoint(seg.p2))) {
				if (p == k) {
					cout << "2nd false" << endl;
				}
				...
				inters = find_intersection_lines(getpoint(p), getpoint(p2),
						getpoint(seg.p1), getpoint(seg.p2));
				if (p == k) {
					cout << "inters " << inters.first << " " << inters.second
							<< endl;
					...
					cout << "seg.p2 " << getpoint(seg.p2).first << " "
							<< getpoint(seg.p2).second << endl;
					cout << "positive int "
							<< positive_intersection(getpoint(p), getpoint(p2),
									inters) << endl;
					cout << "point in seg"
							<< point_in_segment(inters, getpoint(seg.p1),
									getpoint(seg.p2)) << endl;
...
void initiate_status_segment(set<status_segment>& status, int p, int obs1,
		int obs2, coord infinity_point) {
	/*if(p==7) {
	...
	if (edges_intersect(getpoint(p), infinity_point, getpoint(obs1),
			getpoint(obs2))
			&& compute_angle(getpoint(obs1), getpoint(p), infinity_point) != 0
			&& compute_angle(getpoint(obs2), getpoint(p), infinity_point)
					!= 0) {
		status_segment edg;
		edg.p1 = obs1;
		edg.p2 = obs2;
...
void print_vector(vector<event> vec) {
	for (int i = 0; i < vec.size(); ++i) {
		cout << " p " << vec[i].p << " angle: " << vec[i].angle << " p1: "
				<< vec[i].segment.p1 << " p2: " << vec[i].segment.p2
...
void visibility_point(graph& visibility, int p) {
	...
	if (p == k) {
		cout << "point: " << p << endl;
		cout << "point: " << getpoint(p).first << " " << getpoint(p).second
				<< endl;
	}
	...
	if (p == k) {
		...
		for (set<status_segment>::iterator it = status.begin();
				it != status.end(); ++it) {
	...
	for (set<int>::iterator it = visible_points.begin();
			it != visible_points.end(); ++it) {
		if (p != *it) {
...
void visibility_point_obstacle(graph& visibility, int p, int previous,
		int next) {
	int k = -20;
	if (p == k) {
		cout << "obstacle point: " << p << endl;
		cout << "obstacle point: " << getpoint(p).first << " "
				<< getpoint(p).second << endl;
		cout << "previous: " << previous << " next: " << next << endl;
	}
	vector<event> events(2 * pr.nobs);
	double max_angle = compute_angle(getpoint(previous), getpoint(p),
			getpoint(next));
	if (p == k) {
	...
	int mult = 1000;
	coord infinity_point = make_pair(
			mult * getpoint(previous).first + (1 - mult) * getpoint(p).first,
			mult * getpoint(previous).second + (1 - mult) * getpoint(p).second);
...
graph insert_obstacle_edges() {
	...
	if (first) {
		...
		visibility_point_obstacle(visibility, pr.n, pr.n + pr.nobs - 1,
				pr.n + 1);
		for (int i = 1; i < pr.nobs - 1; ++i) {
			visibility_point_obstacle(visibility, pr.n + i, pr.n + i - 1,
					pr.n + i + 1);
		}
		visibility_point_obstacle(visibility, pr.n + pr.nobs - 1,
				pr.n + pr.nobs - 2, pr.n);
	...
	if (!first) {
		for (int i = 0; i < obstacle_visibility.size(); ++i) {
			for (set<int>::iterator it = obstacle_visibility[i].begin();
					it != obstacle_visibility[i].end(); ++it) {
				vis[i].insert(*it);
			}
		}
	}
		...
		if (dist == -1 || dist > pr.t * edges[i].dist) {
			if (edges[i].straight_line) {
				...
				for (int j = 1; j < shortest_path.size(); ++j) {
					insert_edge(tspanner, shortest_path[j - 1],
							shortest_path[j]);
				}
			}
		}
...
void writeFile(string filename, graph& graph, result& res) {
	...
	for (int i = 0; i < graph.size(); ++i) {
		for (set<int>::iterator it = graph[i].begin(); it != graph[i].end();
				++it) {
			outputFile << " " << *it;
		}
		outputFile << endl;
...
result computeResultParameters(graph& tspanner, vector<edge>& edges) {
	...
	for (int i = 0; i < tspanner.size(); ++i) {
		for (set<int>::iterator it = tspanner[i].begin();
				it != tspanner[i].end(); ++it) {
			++size;
...
map<string, vector<coord>> split_points(vector<coord> points,
	...
	if (points.size() <= 1) {
		...
		// check if all points are the same- Stack Overflow fix
...
void constructWSPD(Quadtree* cell1, Quadtree* cell2, float s) {
	...
	if (cell1 != NULL && cell2 != NULL) {
		...
		if (point_inside_obstacle(additional_points[0])
				|| point_inside_obstacle(additional_points[1])) {
			...
			dist = euclidean_distance(cell1->boundary.at(4),
					cell2->boundary.at(4));
		}
		...
		if (dist - cellDiameter1 / 2 - cellDiameter2 / 2
				>= s * (largerDiameter / 2)) {
...
void build_tspanner(graph& tspanner) {
	...
	for (size_t i = 0; i < well_separated_pairs.size(); i++) {
		...
		int p1 = find(pr.points.begin(), pr.points.end(), a.front())
				- pr.points.begin();
		int p2 = find(pr.points.begin(), pr.points.end(), b.front())
				- pr.points.begin();
...
void execute_greedy(string filename) {
	...
	double execution_timer = (clock() - tStart)
			/ (double) (CLOCKS_PER_SEC / 1000);

	result res = computeResultParameters(tspanner, all_edges_sorted);
	res.execution_time = execution_timer;
	cout << "result: " << res.dilation << " " << res.size << " " << res.weight
			<< " " << res.execution_time << endl;
